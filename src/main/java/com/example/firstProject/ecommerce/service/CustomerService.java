...
public class CustomerService {
	...
	public Customer updateCustomer(Long id, UpdateCustomerRequest request) {
		...
		String incomingEmail = request.email();
		if (incomingEmail != null && !Objects.equals(customer.getEmail(), incomingEmail)) {
			if (customerRepository.findByEmail(incomingEmail).isPresent()) {
				throw new BadRequestException("Customer email already exists: " + incomingEmail);
			}
			customer.setEmail(incomingEmail);
		}
